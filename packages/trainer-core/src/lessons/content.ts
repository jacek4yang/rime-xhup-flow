...
export const LEARN_CHAPTERS: readonly LearnChapter[] = [
    ...
    sections: [
      {
        kind: "text",
        paragraphs: [
          "全码(4 码)是整个体系的确定性地基:音 2 码 + 形 2 码。规范 core 与有来源扩展字共享同一生成管线，全部静态 exact 菜单经过冻结审计——你今天练的编码和明年输入法里的编码完全一致,肌肉记忆永不作废。",
  ...
  },
  {
    ...
    level: "advanced",
    title: "词语编码与多级简码",
    summary: "逐字双拼出全码,canonical v2 把常用词压到更短的简码。",
    sections: [
      {
        kind: "text",
        paragraphs: [
          "词语的全码是逐字双拼的拼接:二字词 4 键(如「时间」= uijm),三字词 6 键,四字词 8 键。词库随方案一起安装、一起冻结,不需要联网更新。",
          "在词全码之上,本方案维护一套 optimizer v2 canonical 映射:PRIMARY 保留两键、传统 IF 等各类简码及它们在混排菜单中的名次;FIXED_FIRST 保留占用码上符合单调模式的首选简码。练习器对简码条目同样接受「简码 / 全码」双路线。",
  ...
  },
  {
    ...
    level: "basic",
    title: "常见问题与自查",
    summary: "简码忘了、某字打不出、候选顺序——先来这里查。",
    sections: [
      {
        kind: "list",
        heading: "高频问题",
        items: [
          "简码忘了怎么办?——所有简码条目的备用合法码都是它的全码,直接打全码即可上屏;简码会在错题中心和本章练习里自然回炉。",
          "某个字三码出不来?——三码只覆盖区分度高的常用字;补上次形打四码,规范单字全覆盖。",
          "候选顺序为什么是这样?——静态候选(全码/简码/固定词)永远优先且顺序冻结;动态学习候选只排在全部静态候选之后,不会顶掉你练成的编码。",
          "形键想不起来?——到上一章的形码探索器点对应键,看高频例字;或直接打全码,不影响输入。",
          "怎么切换流畅/固定模式?——在输入法的方案选单中切换(默认快捷键 F4 或 Ctrl+`);两套方案的编码数据完全一致。",
          "学习数据存在哪、会丢吗?——本机 xhup_flow_user.userdb;控制中心可导出快照/恢复/重置,重装或升级方案不会删除学习数据。",
        ],
      },
      {
        kind: "text",
        paragraphs: [
          "自查顺序建议:先看练习页的实时提示(答错会显示正确编码),再到错题中心看是不是系统性弱点,最后到统计页看准确率与键/字——如果准确率正常而键/字偏高,说明该补简码而不是补速度。",
        ],
      },
    ],
  },
