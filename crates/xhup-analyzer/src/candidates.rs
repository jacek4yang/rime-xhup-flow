...
pub fn enumerate_targets(words: &[WordCodeAnalysisEntry]) -> (Vec<WordTarget>, EnumerationStats) {
    ...
    for entry in words {
        ...
        let mut seen: BTreeSet<KeySequence> = BTreeSet::new();
        ...
        for mask in 1usize..(1usize << char_count) {
            ...
            // 不同模式可能投影出相同码(相邻键相同):按 (词, 码) 去重,先枚举到者胜。
            if !seen.insert(shortcut_code.clone()) {
                continue;
            }
            let mode = ShortcutMode(modes.into_boxed_slice());
            *stats.by_length.entry(length).or_default() += 1;
            *stats.by_pattern.entry(mode.pattern()).or_default() += 1;
