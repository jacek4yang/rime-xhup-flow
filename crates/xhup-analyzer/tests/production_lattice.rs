...
fn production_lattice_contains_all_three_source_kinds() {
    ...
    // 研究(yjjq) + 生命(ugmk);研究生(yjjqug) 为 6 键前缀词。
    let built = build_production_lattice("yjjqugmk", LIMIT);
    let stats = built.stats();
    // 多来源事实流:单字原语 + hot 词 + 扩展词全部进入:
    assert!(stats.facts > 0);
    assert!(stats.edges > 0);
    // 三层词表按设计互斥(base / extended / 搜狗增量),同一 (span, text)
    // 跨层重合在当前数据架构下不会出现;跨源融合契约由 xhup-decoder
    // 的 LatticeBuilder 单元测试覆盖(fusion 5 例)。此处验证三类来源
    // 的事实都能经桥接进入边集合,且没有产生重复语义路径。
...
#[test]
fn hot_frequency_evidence_dominates_rank_projection() {
    let built = build_production_lattice("yjjqugmk", LIMIT);
    // (0,4) "研究" 来自 hot 静态层(base 词表逐字双拼码 yjjq):
    let edge = built
        .lattice()
        .edges()
        .iter()
