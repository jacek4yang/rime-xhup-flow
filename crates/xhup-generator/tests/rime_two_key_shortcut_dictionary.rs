//! 二码零冲突词语简码 Rime 词典(`xhup_flow_two_key_shortcuts.dict.yaml`)
//! 的生成测试:行数、权重、码合法性、唯一性、时间哨兵缺席与字节级确定性。

use std::collections::BTreeSet;

use xhup_generator::{
    canonical_two_key_shortcut_entries, generate_rime_two_key_shortcut_dictionary,
};
...
fn rows_match_canonical_entry_count() {
    let dict = generate_rime_two_key_shortcut_dictionary();
    assert_eq!(
        body_rows(&dict).len(),
        canonical_two_key_shortcut_entries().len(),
        "词典行数应等于 canonical 二码简码条数"
    );
}
...
fn every_row_is_word_code_weight_one() {
    let dict = generate_rime_two_key_shortcut_dictionary();
    let canonical: BTreeSet<(String, String)> = canonical_two_key_shortcut_entries()
        .iter()
    ...
    for row in body_rows(&dict) {
        ...
        assert_eq!(fields[1].chars().count(), 2, "码长度为 2: {row}");
        assert!(
...
fn dictionary_header_matches_conventions() {
    ...
    for line in [
        "# Rime dictionary",
        "name: xhup_flow_two_key_shortcuts",
