...
fn rows_match_canonical_entry_count() {
    ...
    assert_eq!(
        ...
        "词典行数应等于 canonical PRIMARY 条数"
    );
}

#[test]
fn every_row_matches_primary_with_unique_merged_weight() {
    let dict = generate_rime_word_shortcut_dictionary();
    let canonical: BTreeSet<(String, String, u32)> = canonical_primary_shortcut_entries()
        .iter()
        .map(|e| {
            (
                e.word().to_string(),
    ...
    let mut seen_code_weights = BTreeSet::new();
    for row in body_rows(&dict) {
        let fields: Vec<&str> = row.split('\t').collect();
        assert_eq!(fields.len(), 3, "每行 词<TAB>码<TAB>权重: {row}");
        let weight: u32 = fields[2].parse().expect("权重应为正整数");
        assert!(weight >= 1, "权重应为正整数: {row}");
        assert!(
            fields[1].chars().all(|c| c.is_ascii_lowercase()),
            "码为纯小写 a-z: {row}"
        );
        assert!((2..=5).contains(&fields[1].len()), "码长度在 2..=5: {row}");
        assert!(
        ...
        assert!(
            seen_code_weights.insert((fields[1], weight)),
            "同码权重重复: {row}"
        );
    }
}
