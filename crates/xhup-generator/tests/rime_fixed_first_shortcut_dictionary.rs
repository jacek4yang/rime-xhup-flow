//! FIXED_FIRST 词语简码 Rime 词典
//! (`xhup_flow_fixed_first_shortcuts.dict.yaml`)的生成测试:行数、权重、
//! 码合法性、唯一性、时间哨兵与字节级确定性。

use std::collections::BTreeSet;

use xhup_generator::{
    canonical_fixed_first_shortcut_entries, generate_rime_fixed_first_shortcut_dictionary,
};
...
fn rows_match_canonical_entry_count() {
    ...
    assert_eq!(
        ...
        "词典行数应等于 canonical FIXED_FIRST 简码条数"
    );
}
...
fn dictionary_header_matches_conventions() {
    ...
    for line in [
        "# Rime dictionary",
        "name: xhup_flow_fixed_first_shortcuts",
