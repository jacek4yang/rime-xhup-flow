...
//! 顶层词典(`xhup_flow`)、方案(`xhup_flow`)与训练器数据集
//! (`xhup_flow_trainer.json`)的纯内存生成:不读写文件,也不读取任何既有
//! Rime 词典。静态单字条目由唯一管线最终化(推导 → 去重 → 万象读音分数
//! 聚合 → 组内排名 → 显式 Rime 权重),Rime 词典与训练器 JSON 都是同一份
//! 最终化数据的投影;候选排名由显式权重表达,行/条目输出顺序仅是确定性的
//! 序列化顺序。在相同规范数据、相同频率数据、相同 xhup-generator 源码
//! (含其 package version)与相同模板下,生成结果字节级一致。
#![forbid(unsafe_code)]
