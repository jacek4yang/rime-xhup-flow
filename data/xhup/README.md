...
- `official_char_code_oracle.tsv`：通过小鹤官网查形页及其
  ...
  不篡改为语言学拼音。
- `flypy_official_char_codes.tsv`：**官网权威字形全量快照层**(2026-09-14
  新增)。从小鹤官网查形页数据集 `ixdata.json` 确定性导出的 8,414 字
  `字/全码/二级拆分/三级拆分/读音` 完整事实,payload SHA-256 与页面标记
  (`-`/`*`/`+`)语义记录在 TSV 注释头。与 `attested_char_codes.tsv` 的
  差异:本层保留官网原始文本(拆分序列、多读音、标记),不参与生产
  member 投影,而是作为 attested 层的权威参照与后续扩展的机械来源;
  交叉审计发现既有 shape 数据与官网存在 345 处形码差异、官网新增
  239 个 attested 之外的字,后续以官网层为准逐步收敛。
