...
本层由冻结的 legacy-any-fi-v1 语法生成,其中包含大量
**非单调**模式(当前 44,518 条中 13,294 条:`IIF` 4,709、`IFI` 3,570、
`IF` 3,042、`IFF` 1,478、`IIIF` 437、`FIF` 39、`IFII` 14、`IIFI` 5;
碰撞共存修复重生成前的 v0.1.0-rc.0 为 44,448/13,281)。

这些映射一旦随正式 v1.0.0 发布即成为**用户肌肉记忆,无限期保持支持**,
不删除、不「清理」、不由新语法重算;v1.0.0 之前的 RC 阶段映射仍允许
经评审的数据演进(如碰撞共存修复的级联重生成)。新的
