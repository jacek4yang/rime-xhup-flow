...
| 指标 | baseline | 上下文 | 变化 |
|---|---|---|---|
| 期望候选选择成本 | 0.0724 键/token | **0.0210 键/token** | **−71%** |
| 节省 | — | — | **+0.0513 键/token** |

选择成本标度与 `replay::ReplayCostModel` 同源(rank1 = 0,rank2 = 0.5 键,
rank3 = 1.0 键,其余 = 2.0 键;期望词不在菜单时按 2.0 键计,**不当作免费**)。

这回答了 §1 的核心质疑:上下文不只把 rank1 命中率从 88.76% 提到 96.61%,
还**把期望选择成本降到原来的 29%** —— 收益不是「换了个指标好看」,而是
真实的用户按键节省。

## 实测结论 6:证据阈值闸门会降低净收益(负面结果,已否决)

§6 要求「弱证据场景确定性平滑降级至静态层排序」。自然实现是加一个**证据
优势阈值**:当上下文候选相对 baseline 首选的转移证据优势不足 `margin` 时,
拒绝重排。本 PR 对该策略做了真实数据敏感度扫描(2701 个歧义 token):

| margin | reorders | gain | harm | net |
|---|---|---|---|---|
| **0(无闸门)** | 470 | **458** | 8 | **+0.1666** |
| 1 | 470 | 308 | 8 | +0.1111 |
| 4 | 470 | 275 | 8 | +0.0989 |
| 16 | 470 | 193 | 8 | +0.0685 |
| 32 | 470 | 143 | 8 | +0.0500 |
...
**结论:阈值单调地摧毁收益,而 harm 只在 margin ≥ 128 时才归零。**

- 在**唯一**能把 harm 归零的工作点(margin ≥ 128)上,净收益从 +0.1666
  掉到 +0.0281 —— **放弃了 83% 的收益**来消除 8 个错误。
- margin=1 就立刻损失 150 个正确重排,说明正确重排里大量是「证据优势很小
  但方向正确」的样本 —— 它们与错误重排**在证据强度上不可分**。

另一个关键读数:**470 次重排里 458 次是对的(98.3%)**。上下文重排的
错误率是 `8/470 = 1.7%`,而不是 `8/5729 = 0.14%`。

### 因此(§6 的落地方式修正)

- **不新增证据优势阈值闸门** —— 实测它会显著降低产品净收益;
- 真正需要的是**更强的证据源**(能区分「他的/它的」这类同音近义对),
  或针对**同音近义对**的显式保守策略(要求极高优势才重排),而不是一刀切
  的全局阈值;
- 该结论已固化为测试断言(`harmful_diagnostics_expose_evidence_not_near_ties`),
  防止后续代理重新引入被否决的阈值策略。
