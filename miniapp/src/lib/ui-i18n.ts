/**
 * 小程序本地增量翻译表(纯 TS,无 React / Taro 依赖,node 测试可直接运行)。
 *
 * 统计页与备份导入页的新增文案不进共享核心 i18n 表(共享表由多端共用,
 * 另行统一迁移),在此以 zh 为基准、en 强制键对齐的方式维护。
 * `translateUi` 先查本地增量表,再回落共享核心表;插值 {n} 占位与核心一致。
 */
...
export function translateUi(
  ...
  if ((key as string) in zhExtra) {
    return interpolate(
      EXTRA_DICTIONARIES[language][key as UiExtraKey],
      params ?? {},
    );
  }
